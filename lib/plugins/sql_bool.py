...
class SQLBool(object):
        ...
        for false_request, true_request, true_request_1 ,false_request_2, true_request_2, false_request_3, true_request_3, false_request_4, true_request_4, true_request_4_1, false_request_5, true_request_5, true_request_5_1,false_request_double_quotes,true_request_double_quotes, true_request_double_quotes_1,false_request_double_quotes_2,true_request_double_quotes_2 in zip(
            ...
            if (false_score <= self.similar and true_score > self.similar) or (false_score <= self.similar and true_score_1 > self.similar) or (false_score <= self.similar and true_score_1_2 >self.similar and false_score_1_2<=self.similar and verify_equal):
                ...
                if false_score_2 <= self.similar and true_score_2 > self.similar:
                    ...
                    # sql="insert into sql_bool (`host`,`first_req`,`first_resp`,`bool_true_req`,`bool_true_resp`, `bool_false_req`, `bool_false_resp`, `payload`, risk) VALUES ('{}','{}','{}','{}','{}','{}','{}','{}',{})".format(
                    ...
                    #         escape_string(self.payload_str(self.bool_str_tuple_second, false_score_2, true_score_2)),
                    #         1)
                    sql = """INSERT INTO sql_bool (host, first_req, first_resp, bool_true_req, bool_true_resp, bool_false_req, bool_false_resp, payload, risk)VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"""
                    params = (
                        request_data['host'],
                        escape_string(req_raw_first),
                        escape_string(resp_raw_first),
                        escape_string(self.wapper.generateRequest(true_request_2)),
                        escape_string(true_response_2),
                        escape_string(self.wapper.generateRequest(false_request_2)),
                        escape_string(false_response_2),
                        escape_string(self.payload_str(self.bool_str_tuple_second, false_score_2, true_score_2)),
                        1
                    )
                    DBsqli.insert_exec(sql, params)
                else:
                    ...
                    if false_score_3 <= self.similar and true_score_3 > self.similar:
                        ...
                        sql = """INSERT INTO sql_bool (host, first_req, first_resp, bool_true_req, bool_true_resp, bool_false_req, bool_false_resp, payload, risk) 
                                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"""
                        params = (
                            request_data['host'],
                            escape_string(req_raw_first),
                            ...
                            escape_string(self.wapper.generateRequest(false_request_3)),
                            escape_string(false_response_3),
                            escape_string(self.payload_str(self.bool_str_tuple_third, false_score_3, true_score_3)),
                            1
                        )
                        DBsqli.insert_exec(sql, params)
                        ...
                        params = (
                            request_data['host'],
                            req_raw_first,  # 不需要手动转义
                            resp_raw_first,  # 不需要手动转义
                            self.wapper.generateRequest(true_request),  # 不需要手动转义
                            true_response,  # 不需要手动转义
                            self.wapper.generateRequest(false_request),  # 不需要手动转义
                            false_response,  # 不需要手动转义
                            self.payload_str(self.bool_str_tuple, false_score, true_score) +
                            " === " + str(true_score_1) +
                            " === " + str(true_score_1_2),  # 不需要手动转义
                            0
                        )
                        DBsqli.insert_exec(sql,params)
                ...
                if (false_score_4 <= self.similar and true_score_4 > self.similar) or (false_score_4 <= self.similar and true_ture_score>self.similar and true_false_score<self.similar):
                    ...
                    if true_score_4_1 > self.similar:
                        ...
                        params = (
                            request_data['host'],
                            req_raw_first,  # 保持原变量名
                            resp_raw_first,  # 保持原变量名
                            self.wapper.generateRequest(true_request_4_1),  # 保持原变量名
                            true_response_4_1,  # 保持原变量名
                            self.wapper.generateRequest(false_request_4),  # 保持原变量名
                            false_response_4,  # 保持原变量名
                            self.payload_str(self.bool_int_tuple, false_score_4, true_score_4_1),  # 保持原变量名
                            1  # 保持原变量名
                        )
                        DBsqli.insert_exec(sql,params)
                    else:
                        ...
                        params = (
                            ...
                            req_raw_first,  # 保持原变量名
                            resp_raw_first,  # 保持原变量名
                            self.wapper.generateRequest(true_request_4),  # 保持原变量名
                            true_response_4,  # 保持原变量名
                            self.wapper.generateRequest(false_request_4),  # 保持原变量名
                            false_response_4,  # 保持原变量名
                            self.payload_str(self.bool_int_tuple, false_score_4, true_score_4) + " === " + str(
                                true_ture_score) + " -> " + str(self.bool_int_tuple[1]) + ":::" + str(
                                self.bool_int_tuple[2]),  # 保持原变量名
                            0  # 保持原变量名
                        )
                ...
                if false_score_5 <= self.similar and true_score_5 > self.similar:
                    ...
                    if true_score_5_1 > self.similar:
                        ...
                        params = (
                            ...
                            req_raw_first,  # 保持原变量名
                            resp_raw_first,  # 保持原变量名
                            self.wapper.generateRequest(true_request_5_1),  # 保持原变量名
                            true_response_5_1,  # 保持原变量名
                            self.wapper.generateRequest(false_request_5),  # 保持原变量名
                            false_response_5,  # 保持原变量名
                            self.payload_str(self.bool_order_tuple, false_score_5, true_score_5_1),  # 保持原变量名
                            1  # 保持原变量名
                        )
                        ...
                        params = (
                            ...
                            req_raw_first,  # 保持原变量名
                            resp_raw_first,  # 保持原变量名
                            self.wapper.generateRequest(true_request_5),  # 保持原变量名
                            true_response_5,  # 保持原变量名
                            self.wapper.generateRequest(false_request_5),  # 保持原变量名
                            false_response_5,  # 保持原变量名
                            self.payload_str(self.bool_order_tuple, false_score_5, true_score_5),  # 保持原变量名
                            0  # 保持原变量名
                        )
                ...
                if (false_score_double_quotes <= self.similar and true_score_double_quotes > self.similar) or (
                    ...
                    if false_score_double_quotes_2 <= self.similar and true_score_double_quotes_2 > self.similar:
                        ...
                        params = (
                            ...
                            req_raw_first,  # 保持原变量名
                            resp_raw_first,  # 保持原变量名
                            self.wapper.generateRequest(true_request_double_quotes_2),  # 保持原变量名
                            true_response_double_quotes_2,  # 保持原变量名
                            self.wapper.generateRequest(false_request_double_quotes_2),  # 保持原变量名
                            false_response_double_quotes_2,  # 保持原变量名
                            self.payload_str(self.bool_double_quotes_tuple_second, false_score_double_quotes_2,
                                             true_score_double_quotes_2),  # 保持原变量名
                            1  # 保持原变量名
                        )
                        DBsqli.insert_exec(sql,params)
                        ...
                        self.db.save(
                            "insert into sql_bool (`host`,`first_req`,`first_resp`,`bool_true_req`,`bool_true_resp`, `bool_false_req`, `bool_false_resp`, `payload`, risk) VALUES ('{}','{}','{}','{}','{}','{}','{}','{}',{})".format(
                                request_data['host'], escape_string(req_raw_first), escape_string(resp_raw_first),
                                escape_string(self.wapper.generateRequest(true_request_double_quotes)),
                                escape_string(true_response_double_quotes),
                                escape_string(self.wapper.generateRequest(false_request_double_quotes)),
                                escape_string(false_response_double_quotes),
                                escape_string(self.payload_str(self.bool_double_quotes_tuple, false_score_double_quotes,
                                                               true_score_double_quotes) + " === " + str(
                                    true_score_double_quotes_1) + " === " + str(true_score_double_quotes_1_2)), 0))
            ...
            if len(set({strFlag, intFlag, orderFlag, strdouble_quotesFlag})) == 1:
                ...
                if resp_raw_first_no_header != self.no_header(resp_raw_repeat):
                    ...
                    if self.calculation(bool_str_list) == [1]:
                        ...
                        sql = """INSERT INTO sql_bool (host, first_req, first_resp, bool_true_req, bool_true_resp, payload, risk) 
                                 VALUES (?, ?, ?, ?, ?, ?, ?)"""
                        ...
                        params = (
                            ...
                            req_raw_first,  # 保持原变量名
                            resp_raw_first,  # 保持原变量名
                            self.wapper.generateRequest(true_request_2),  # 保持原变量名
                            resp,  # 保持原变量名
                            self.payload_rdm(self.bool_rdm[0]),  # 保持原变量名
                            0  # 保持原变量名
                        )
                        ...
                        if self.calculation(bool_int_list) == [1]:
                            ...
                            params = (
                                ...
                                self.wapper.generateRequest(true_request_4_1),  # 保持原变量名
                                resp_3,  # 保持原变量名
                                self.payload_rdm(self.bool_rdm[1]),  # 保持原变量名
                                0  # 保持原变量名
                            )
                            ...
                            if self.calculation(bool_order_list) == [1]:
                                ...
                                params = (
                                    ...
                                    self.wapper.generateRequest(true_request_5_1),  # 保持原变量名
                                    resp_o_3,  # 保持原变量名
                                    self.payload_rdm(self.bool_rdm[2]),  # 保持原变量名
                                    0  # 保持原变量名
                                )
    ...
    def fetch_sql2(self):
        ...
        sql = 'SELECT create_time, host, risk, payload, bool_true_req, bool_true_resp FROM sql_bool WHERE date(create_time) = date("now")'
        items = DBsqli.GetSelect(sql)
