...
from lib.rules.rules_sqli import Get_sql_errors,GetSqlErr
                ...
                if index not in self.sql_index: # 判断是否是存在的index 下标，不是才探测。保证同一个参数如果有探测到sql之后，后面就不探测这个参数
                    ...
                    for sql_regex, dbms_type in self.sql_error:
                        ...
                        if match:
                            ...
                            # print(match.group(0))
                            # print(val['param_in_url']['id'])

                            flag = True
                            self.sql_index.append(index)  # 如果该请求包判断存在注入，就加入list，下次不探测这个index
                            self.logger.critical('[+] 发现SQL报错注入, {}'.format(match.group()))
                            vals=val['host']+"--注入参数POC："+val['param_in_url']['id']
                            dbms_types=dbms_type + "--匹配的关键字：" + match.group(0)

                            DBsqli.insert_err(escape_string(self.wapper.generateRequest(val)), escape_string(resp_raw),vals,dbms_types)
                            # DBsqli.insert_err(escape_string(self.wapper.generateRequest(val)), escape_string(resp_raw),['host'], dbms_type)
                            ...
                            break
        self.logger.info("--------------error_sql_str-END-----------------")
        if flag:
            self.fetch_sql2()
        self.logger.info('[*] SQL注入探测完成, 共发送 {} 个请求'.format(self.request_count))

    def fetch_sql2(self):
        sql = 'SELECT create_time, host, dbms, request_data, response FROM sql_error WHERE date(create_time) = date("now")'
